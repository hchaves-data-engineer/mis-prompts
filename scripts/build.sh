...
cd "$project_dir"
mkdir -p .build/App.noindex dist
...
# File Provider may add metadata to the bundle or its resources.
xattr -cr "$app_dir"
codesign --force --sign - "$app_dir"
codesign --verify --deep --strict "$app_dir"
ditto --norsrc --noextattr "$app_dir" ".build/App.noindex/Mis Prompts.app"
zip_path="dist/Mis-Prompts-1.1-macOS-${build_arch}.zip"
ditto -c -k --keepParent --norsrc --noextattr "$app_dir" "$zip_path"
printf 'Aplicación: %s/.build/App.noindex/Mis Prompts.app\nArchivo: %s/%s\n' "$project_dir" "$project_dir" "$zip_path"
